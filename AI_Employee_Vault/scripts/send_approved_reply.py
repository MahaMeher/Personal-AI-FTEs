#!/usr/bin/env python3
"""
Send approved email reply from APPROVED folder.
"""
...
APPROVED_FILE = "D:\\Personal-AI-FTEs\\AI_Employee_Vault\\Approved\\EMAIL_REPLY_19d0c9460f2cf5f6.md"
IN_PROGRESS_FILE = "D:\\Personal-AI-FTEs\\AI_Employee_Vault\\In_Progress\\EMAIL_19d0c9460f2cf5f6_.md"
...
def main():
    print("=" * 60)
    print("Executing: APPROVED/EMAIL_REPLY_19d0c9460f2cf5f6.md")
    print("=" * 60)
    ...
    report_filename = f"EMAIL_EXECUTION_REPORT_19d0c9460f2cf5f6.md"
    report_file = f"D:\\Personal-AI-FTEs\\AI_Employee_Vault\\{report_filename}"
...
## Approved Action
- **File**: EMAIL_REPLY_19d0c9460f2cf5f6.md
...
""")
    ...
    if results["status"] == "success":
        ...
        dst = "D:\\Personal-AI-FTEs\\AI_Employee_Vault\\Done\\EMAIL_REPLY_19d0c9460f2cf5f6.md"
        try:
        ...
        # Update In_Progress file status
        try:
            with open(IN_PROGRESS_FILE, 'r') as f:
                in_prog_content = f.read()
            in_prog_content = in_prog_content.replace('status: pending', 'status: completed')
            in_prog_content = in_prog_content.replace('- [ ] Reply to sender', '- [x] Reply to sender')
            with open(IN_PROGRESS_FILE, 'w') as f:
                f.write(in_prog_content)
            print(f"✓ Updated In_Progress file status")
        except Exception as e:
            print(f"⚠️ Could not update In_Progress file: {str(e)}")
